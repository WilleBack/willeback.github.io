	...
	<body style="font-family:Verdana; font-size:8pt;margin:0cm; ">
		...
		<div class="container">
			<xsl:for-each select="catalog/fragment">
				<div class="fragment">
					<div id ="header" style="background-color:Maroon; box-sizing:border-box; width:100%; float:left; color:white; ">
					...
					<div style="width:100%; box-sizing:border-box; padding:0.3em; background:linear-gradient(to right, GhostWhite, Ghostwhite,White);float:left;">
						...
						<div style="width:100%; box-sizing:border-box; padding:0.3em; text-indent:-1em; padding-left:2.3em; float:left;">
							<b><xsl:value-of select="manifestation/action" /></b> <xsl:text> </xsl:text><xsl:value-of select="manifestation/subaction" /><xsl:text> with <i>Trigger: </i><!--</xsl:text><xsl:value-of select="manifestation/trigger" /> -->
						</div> 
					</div> 
