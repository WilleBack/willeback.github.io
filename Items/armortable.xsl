...
<xsl:template name="armordesc">
	<div class="armor">
		...
		<div class="stats" style="width:100%; margin:0.3em; float:left;">
			<b>Armor Bonus: </b> <xsl:text>+</xsl:text><xsl:value-of select="bonus" /><xsl:text>; </xsl:text> <b>Check Penalty: </b> <xsl:text>-</xsl:text><xsl:value-of select="penalty" /><xsl:text>; </xsl:text> <b>Speed Penalty: </b> <xsl:text>-</xsk:text><xsl:value-of select="speed" /> <br />
